...
for i in range(0,S_round): #i equal -> per round
    ...
    for j in range(K_client_num):#j equal per clients -> 아직은 샤드2개 300짜리 다른 숫자라벨2개를 100명한테 주는거다.
        clients_history.append(clients_model[j].fit(np.concatenate([x_train[list_added_label_idx[j][0]:list_added_label_idx[j][0]+300], x_train[list_added_label_idx[j][1]:list_added_label_idx[j][1]+300]], axis=0)
                                                    ,np.concatenate([y_train[list_added_label_idx[j][0]:list_added_label_idx[j][0]+300], y_train[list_added_label_idx[j][1]:list_added_label_idx[j][1]+300]], axis=0), 
                                                    validation_split=0.25, epochs=E_epoch, batch_size=B_batch, verbose=0) )
        if j%10==0:
